defmodule CustomerFeedback.Elasticsearch.ContextTest do
  ...
  describe "#create_feedback_document" do
    ...
    test "Return {:error, binary} with explanation of the error if params is invalid", %{
      ...
      assert {:error, ^error_message} = Context.create_feedback_document(params, post_document_mock)

      params = %{evaluation: -8, title: 3, product_url: "https://localhost:4000/awesome/kettle", customer_id: customer_id}
      error_message = "evaluation: must be greater than or equal to 0; title: is invalid"
      assert {:error, ^error_message} = Context.create_feedback_document(params, post_document_mock)

      params = %{evaluation: 11, text: "", product_url: "https://localhost:4000/awesome/kettle", author: 7}
      error_message = "author: is invalid; customer_id: can't be blank; evaluation: must be less than or equal to 10"
      assert {:error, ^error_message} = Context.create_feedback_document(params, post_document_mock)
    end
    ...
    test "Return {:error, binary} with explanation if elastic search returned an error", %{
      ...
      assert {:error, "Invalid index name [feedback_documents_SSSS], must be lowercase"} = Context.create_feedback_document(params, post_document_mock)
    end
  end
end
